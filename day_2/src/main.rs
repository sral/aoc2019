...
// beeps angrily about a "1202 program alarm". On the radio, an Elf is already
...
// the two inputs (9 and 10), and the position of the output (3). To handle this
// opcode, you first need to get the values at the input positions:
// position 9 contains 30, and position 10 contains 40. Add
...
// opcode, 2. This opcode works just like the previous, but it multiplies
// instead of adding. The inputs are at positions 3 and 11; these positions
// contain 70 and 50 respectively. Multiplying these produces 3500; this is
...
// computer, the first step is to restore the gravity assist program (your
// puzzle input) to the "1202 program alarm" state it had just before the last
// computer caught fire. To do this, before running the program, replace
// position 1 with the value 12 and replace position 2 with the value 2. What
...
// --- Part Two ---
//
// "Good, the new computer seems to be working correctly! Keep
// it nearby during this mission - you'll probably use it again. Real Intcode
...
const EXPECTED_OUTPUT_PART_TWO: usize = 19690720;
const NOUN: usize = 1;
const VERB: usize = 2;
...
fn execute(program: &mut [usize]) {
    ...
    loop {
        match program[ip] {
            1 => {
                let op_one = program[program[ip + 1]];
                let op_two = program[program[ip + 2]];
                let destination = program[ip + 3];
                program[destination] = op_one + op_two;
                ip += 4;
            }
            2 => {
                let op_one = program[program[ip + 1]];
            ...
            99 => return,
            _ => panic!("Illegal opcode: {}", program[ip]),
        }
    }
}

fn part_one(memory: &[usize]) -> Option<usize> {
    let mut part_one_program = memory.to_vec();
    part_one_program[NOUN] = 12;
    part_one_program[VERB] = 2;
    execute(&mut part_one_program);

    Some(part_one_program[0])
}

fn part_two(memory: &[usize]) -> Option<usize> {
    for noun in 0..100 {
        for verb in 0..100 {
            let mut part_two_program = memory.to_vec();
            part_two_program[NOUN] = noun;
            part_two_program[VERB] = verb;
            execute(&mut part_two_program);
            if part_two_program[0] == EXPECTED_OUTPUT_PART_TWO {
...
fn main() {
    ...
    println!("Part 1 answer: {}", part_one(&memory).unwrap());
    println!("Part 2 answer: {}", part_two(&memory).unwrap());
}
...
mod test {
    ...
    fn part_one_examples_test() {
        ...
        for expected in test_cases.iter_mut() {
            execute(&mut expected.0);
